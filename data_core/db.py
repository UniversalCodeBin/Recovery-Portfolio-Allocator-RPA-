...
class Database:
    ...
    def create_schema(self, conn, reset: bool = False) -> None:
        """Create the target schema + tables (and indexes) from migrations."""
        with conn.cursor() as cur:
            if reset:
                cur.execute(f"DROP SCHEMA IF EXISTS {self.schema} CASCADE")
            cur.execute(f"CREATE SCHEMA IF NOT EXISTS {self.schema}")
            self._set_search_path(cur)
        # Execute each migration file in lexical order within the transaction.
    ...
    # ------------------------------------------------------------------
    def load_frame(self, conn, table: str, df: pd.DataFrame, reset_table: bool = False) -> int:
        ...
        with conn.cursor() as cur:
            self._set_search_path(cur)
            if reset_table:
                cur.execute(f'TRUNCATE TABLE "{self.schema}".{table} RESTART IDENTITY CASCADE')
            stmt = (
                f'COPY "{self.schema}".{table} ({_quoted_cols(cols)}) '
