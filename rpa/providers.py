...
class PaymentEventProvider(ABC):
    @abstractmethod
    def validate_event(self, headers: dict[str, str], body: bytes) -> PaymentFailureEvent: ...


class RecoveryActionExecutor(ABC):
    @abstractmethod
    def execute(self, *, execution_id: str, action_id: str, transaction_id: str) -> dict[str, Any]: ...
...
class SimulatorProvider(RecoveryActionExecutor, RecoveryOutcomeProvider):
    ...
    def execute(self, *, execution_id: str, action_id: str, transaction_id: str) -> dict[str, Any]:
        return {"simulation": True, "execution_id": execution_id, "action_id": action_id,
