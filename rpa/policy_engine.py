...
class PolicyEngine:
    ...
    def initial_resource_state(self, resource_limits: Dict[str, Optional[float]]) -> ResourceState:
        return ResourceState(remaining={
        ...
        *,
        ...
        if not action.enabled:
            return PolicyVerdict(
                transaction_id=transaction_id, action_id=action.action_id,
                decision="BLOCK", rule="action_disabled",
                reason=f"action {action.action_id} is disabled", action_type=action.action_type,
            )
        # 2. blocked action list
        if action.action_id in self.config.blocked_actions or \
           action.action_type in self.config.blocked_actions:
            return PolicyVerdict(
                transaction_id=transaction_id, action_id=action.action_id,
                decision="BLOCK", rule="action_blocked",
        ...
        if self.config.prohibit_combination and not action.is_no_op:
            for combo in self.config.prohibit_combination:
                if action.action_type in combo:
                    return PolicyVerdict(
                        transaction_id=transaction_id, action_id=action.action_id,
                        decision="BLOCK", rule="prohibited_combination",
                        reason=f"action {action.action_type} is in prohibited combination {combo}",
                        action_type=action.action_type,
                    )
        # 4. retry limit
        if action.action_type == "retry":
            if retry_count >= self.config.max_retries_per_transaction:
                return PolicyVerdict(
                    transaction_id=transaction_id, action_id=action.action_id,
                    decision="BLOCK", rule="retry_limit",
        ...
        if incentive_units > 0 and self.config.max_incentive_per_transaction is not None:
            if incentive_units > self.config.max_incentive_per_transaction:
                return PolicyVerdict(
                    ...
                    current_usage=float(incentive_units), action_type=action.action_type,
                )
        # 7. batch resource availability (hard gate)
        if not resource_state.can_consume(action):
            blocked_by = next(
                (k for k, u in action.resource_requirements.items()
                 if u and u > 0 and resource_state.remaining.get(k) is not None
                 and u > resource_state.remaining.get(k) + 1e-9), None)
            return PolicyVerdict(
                transaction_id=transaction_id, action_id=action.action_id,
                decision="BLOCK", rule="resource_exhausted",
                reason=f"insufficient remaining {blocked_by} capacity",
                limit=resource_state.remaining.get(blocked_by),
                current_usage=action.resource_requirements.get(blocked_by, 0.0),
                action_type=action.action_type,
            )
        return PolicyVerdict(
            transaction_id=transaction_id, action_id=action.action_id,
            decision="ALLOW", rule="ok", reason="", action_type=action.action_type,
        )
...
__all__ = ["PolicyEngine", "PolicyConfig", "PolicyVerdict", "ResourceState"]
