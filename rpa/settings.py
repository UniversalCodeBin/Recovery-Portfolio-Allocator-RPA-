...
def get_settings() -> Settings:
    ...
    origins = tuple(x.strip() for x in raw_origins.split(",") if x.strip() and x.strip() != "*")
    settings = Settings(
        mode=mode,
        ...
        db_pool_acquire_timeout_seconds=float(os.getenv("RPA_DB_POOL_ACQUIRE_TIMEOUT_SECONDS", "5")),
        webhook_signing_secret=os.getenv("RPA_WEBHOOK_SIGNING_SECRET") or None,
        webhook_timestamp_tolerance_seconds=int(os.getenv("RPA_WEBHOOK_TIMESTAMP_TOLERANCE_SECONDS", "300")),
        model_artifact_path=os.getenv("RPA_MODEL_ARTIFACT_PATH", "artifacts/models/rpa-recovery-logreg-full-v1"),
        demo_data_path=os.getenv("RPA_DEMO_DATA_PATH", "data"),
    )
    if settings.access_token_ttl_seconds < 60 or settings.access_token_ttl_seconds > 86400:
    ...
    if settings.production:
        ...
        if len(settings.auth_signing_key or "") < 32:
            raise RuntimeError("RPA_AUTH_SIGNING_KEY must contain at least 32 characters")
        if len(settings.webhook_signing_secret or "") < 32:
            raise RuntimeError("RPA_WEBHOOK_SIGNING_SECRET must contain at least 32 characters")
        if not settings.cors_origins:
            raise RuntimeError("RPA_CORS_ORIGINS must contain at least one explicit origin")
    return settings
