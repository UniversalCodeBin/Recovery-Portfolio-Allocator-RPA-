...
_ALLOWED_TRANSITIONS = {
    ExecutionState.PLANNED: {ExecutionState.AUTHORIZED, ExecutionState.BLOCKED, ExecutionState.CANCELLED},
    ExecutionState.AUTHORIZED: {ExecutionState.EXECUTING, ExecutionState.BLOCKED, ExecutionState.CANCELLED},
    ExecutionState.EXECUTING: {ExecutionState.SUCCEEDED, ExecutionState.FAILED, ExecutionState.BLOCKED},
    ExecutionState.SUCCEEDED: set(), ExecutionState.FAILED: set(),
    ExecutionState.BLOCKED: set(), ExecutionState.CANCELLED: set(),
}
...
class ExecutionDecision:
    ...
    def can_execute(self, now: datetime | None = None) -> bool:
        ...
        return (self.state == ExecutionState.AUTHORIZED and self.policy_allowed
                and self.authorized and self.resources_reserved
                and self.decision_expires_at > current)


def transition(decision: ExecutionDecision, target: ExecutionState, *, now: datetime | None = None) -> ExecutionDecision:
