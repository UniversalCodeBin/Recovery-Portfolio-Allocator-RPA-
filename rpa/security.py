...
def issue_access_token(principal: Principal, *, key: str, issuer: str, ttl_seconds: int) -> str:
    if principal.role not in ROLES:
        raise ValueError("unknown role")
    now = int(time.time())
    header = _b64(json.dumps({"alg": "HS256", "typ": "JWT"}, separators=(",", ":")).encode())
    payload = _b64(json.dumps({"sub": principal.user_id, "tid": principal.tenant_id,
        "role": principal.role, "jti": principal.token_id, "iss": issuer,
        "iat": now, "nbf": now, "exp": now + ttl_seconds}, separators=(",", ":")).encode())
    signature = _b64(hmac.new(key.encode(), f"{header}.{payload}".encode(), hashlib.sha256).digest())
    return f"{header}.{payload}.{signature}"


def validate_access_token(token: str, *, key: str, issuer: str, revoked_token_ids: Iterable[str] = ()) -> Principal:
    try:
    ...
    if decoded_header != {"alg": "HS256", "typ": "JWT"} or not hmac.compare_digest(signature, expected):
        raise AuthenticationError("invalid bearer token")
    now = int(time.time())
    required = ("sub", "tid", "role", "jti", "iss", "exp", "nbf")
    if any(not isinstance(claims.get(name), str if name not in {"exp", "nbf"} else int) for name in required):
        raise AuthenticationError("invalid bearer token claims")
    if claims["iss"] != issuer or claims["role"] not in ROLES or claims["exp"] <= now or claims["nbf"] > now:
