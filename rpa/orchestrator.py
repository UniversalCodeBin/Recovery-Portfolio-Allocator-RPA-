...
class BatchResult:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "plans": {name: plan.to_records() for name, plan in self.plans.items()},
            "executions": {
...
class RPABatchOrchestrator:
    ...
    def run_batch(
        ...
        *,
        ...
        predictions_model_id: str | None = None,
        ...
        audit.record_batch(
            ...
            or (prediction_result.model_metadata if prediction_result else {}),
        ...
        for strategy, plan in plans.items():
            try:
                ...
                return self._fail_closed(
                    batch_id, f"execution_failed[{strategy}]: {exc}"
                )

        # -- 6. Verification ----------------------------------------------------
        verifications: dict[str, VerificationResult] = {}
        for strategy, plan in plans.items():
            verif = self.verifier.verify(plan, executions[strategy], actions)
...
def run_batch_on_split(
    ...
    *,
    ...
    strategies: list[str] | None = None,
    orchestrator: RPABatchOrchestrator | None = None,
) -> BatchResult:
