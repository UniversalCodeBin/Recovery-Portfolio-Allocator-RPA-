...
def test_execution_requires_transaction_specific_policy_approval():
    ...
    result = ExecutionSimulator().execute(plan, txns, np.ones((2, 2)), [no_op, retry], verdicts, 7)
...
def test_rule_based_respects_shared_resource_capacity():
    ...
    actions = [no_op, retry]
    txns = pd.DataFrame({
        "transaction_id": ["a", "b"], "amount": [100.0, 100.0],
        "days_overdue": [0, 0], "retry_count": [0, 0],
        "historical_success_rate": [0.0, 0.0],
    })
    ev = EVEngine().compute(txns, np.array([[0.0, 1.0], [0.0, 1.0]]), actions)
    verdicts = [PolicyVerdict(t, a.action_id, "ALLOW", action_type=a.action_type)
                for t in txns.transaction_id for a in actions]
    plan = StrategyRunner().rule_based(ev, txns, ["a", "b"], actions,
                                       {"retry": 1.0}, verdicts)
