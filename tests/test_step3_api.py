...
def test_run_batch(client, demo):
    ...
    r = client.post(
        "/recovery/batch",
        json={"split": "demo", "batch_seed": 0, "transaction_ids": ids},
    )
    assert r.status_code == 200
    body = r.json()
...
def test_run_strategy_ev_greedy(client, demo):
    r = client.post(
        "/recovery/strategy/ev_greedy", json={"split": "demo", "batch_seed": 1}
    )
    assert r.status_code == 200
    body = r.json()
...
def test_compare_fair_comparison(client, demo):
    ...
    r = client.post(
        "/recovery/compare",
        json={"split": "demo", "batch_seed": 2, "transaction_ids": ids},
    )
    assert r.status_code == 200
    body = r.json()
    assert body["simulation"] is True
...
def test_execute_simulation_only(client, demo):
    r = client.post(
        "/recovery/execute",
        json={"split": "demo", "strategy": "rpa_optimizer", "batch_seed": 3},
    )
    assert r.status_code == 200
    body = r.json()
    assert body["simulation"] is True
...
def test_get_batch_and_explanation(client, demo):
    ...
    first_txn = ids[0]
    exp_res = client.get(
        f"/recovery/explain/{batch_id}/{first_txn}?strategy=rpa_optimizer"
    )
    ...
    bad_strat = client.get(
        f"/recovery/explain/{batch_id}/{first_txn}?strategy=invalid_strat"
    )
